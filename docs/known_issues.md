...
| KI-031 | S2 | Transform path uses object-key `pd.Series.map`; non-additive stats still per-fold | `_transform_array` maps encodings via a pandas hash lookup (~52% `get_indexer` of a multi-stat `transform`); the single-pass kernel covers **mean/var/std** only — median/min/max/skew/custom still re-fit per fold (the hybrid OOF slow path). Next CPU lever: factorize-once + numpy **gather** (integer codes) for fit+transform, which also yields integer **joint** codes to unblock GPU `combination` (KI-018). See `docs/notes/2026-06-27-cuml-vs-sklearn-te-levers.md`. |
