...
| KI-018 | S3 | GPU `combination` forced to CPU | CPU now keys combinations by **int64 joint codes** (KI-019, 2026-06-27) instead of tuples — the integer column GPU `combination` needs. Still host-only: unblocking it (drop `len(cols)>1` from `host_only` + build joint codes in `_gpu.py`) is the next PR, with **mandatory** Colab CPU/GPU parity. |
