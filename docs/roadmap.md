...
> laplace_alpha, max_classes/KI-016, device transform-LUT cache) — T4 suite 384 passed. **0.5.2 RELEASED to PyPI (2026-07-02)** — KI-010 parity test + docs refresh + KI-table audit (PRs #28/#29, `v0.5.2` tag, publishing green first-run). **2026-07-02: KI-010 RESOLVED** (exact sklearn auto-smoothing parity, fp-rounding-level; `test_sklearn_auto_parity.py`) **+ docs/API refresh shipped** (package docstring, README, design-doc §3, `examples/stats_and_smoothing.py` in the gate). **Next candidates (no work started):** none queued — pick from KI table (KI-013/014 cuDF string/NaN semantics notes) or new features on request. **Ops nit:** Actions warn on Node 20
> deprecation (bump `actions/checkout@v4` etc.).

## 2026-09-05 WP1 — boundary repairs (Unreleased)
